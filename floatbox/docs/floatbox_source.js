...
// get starting link info
	this.startAnchor = anchor;  // for putting focus back in setVisibility()
	var href = anchor.getAttribute('href');
	var rel = anchor.getAttribute('rel');
	var rev = anchor.getAttribute('rev');
	var title = anchor.getAttribute('title');
...
// ie6 always shows selects on top, doesn't handle position:fixed and doesn't respect height:width:100% against the body
	...
	if (this.ie6 && this.isIframe) this.innerBorder = 0;  // ie6 screws up iframe layout if there's a border
	if (this.hideFlash) {
...
// load image or iframe
	if (this.isIframe) {
		setTimeout(function() { fb.setSize(); }, 20);  // on a timer so the display can update
	} else {
...
// add screen scroll values to left and top
	if (this.getXScroll() || this.getYScroll()) {
		...
		if (this.ie6 || this.ieQuirks) this.stretchOverlay();  // the overlay might be stretching the body
...
Floatbox.prototype.getDisplayHeight = function() {
	...
	// Opera. Body.clientHeight is the right measurement,
	// but opera has a bug that excludes the body border from the reported height
	if (window.opera) {
...
//******************************************************************************/
// setVisibility()
// Hides (or shows) elements so they don't appear above the overlay and floatbox
// (For flash and the ie6 select z-index bug)
//******************************************************************************/
Floatbox.prototype.setVisibility = function(tagName, state, thisWindow) {
	// thisWindow is used to recurse through all frames under the top doc
	if (!thisWindow) {
		// first call?  start with the top window object
		...
		try {
			...
			while (i--) {
				...
				if (!state) els[i].focus();  // ff2/mac helper
			}
		} catch(e) {}
	...
	if (!state && this.startAnchor) this.startAnchor.focus();  // put focus back where it started
};
...
// avoid repeated screen flashes by waiting until browser resizing is complete before redrawing the overlay
	if (arguments.length == 1) {  // it's from a resize event
		...
		fb.objTimeouts.onresize = setTimeout(function() { fb.stretchOverlay(); }, 50);  // set a new one for a bit later
